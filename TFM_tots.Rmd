...
Com a exercici extra en comparació als altres mètodes, s'ha decidit calcular també els quartils. L'error estàndard d'aquests s'ha calculat a partir de la tècnica bootstrap i la inferència només s'ha fet a 1 cua. Aquests permeten veure si realment hi ha diferències entre aquells individus que s'exposen molt al risc, és a dir, que per exemple es troben molta part del temps conduint per sobre la velocitat permesa. 

Els resultats mostren clarament que cap de les variables dependents dona lloc a un pvalor significatiu, raó per la qual no es pot dir res sobre aquests estimadors. Aquests resultats no sorprenen en haver detectat al llarg de tot el projecte que els estimadors no paramètrics o semi-paramètrics no són una bona opció per a aplicar a aquestes dades. Concretament, el problema està en que aquestes són massa desbalancejades, és a dir, existeixen molts més usuaris a la base de dades que no han tingut cap accident, que usuaris amb accidents, dificultant molt que models d'aquest tipologia puguin distinguir correctament. 

Pel que fa als quartils aquests altre cop han donat lloc a coeficients no significatius. Tanmateix, sí que es nota que a mesura que augmentem el quartil, augmenta el valor del coeficient en valor absolut en la majoria dels cassos. Això equival a dir que el número de quilòmetres (pel cas de la variable `Total`), augmenta pels usuaris que no han declarat cap accident durant el segon període, que és el resultat que s'esperia pensant en sentit comú, ja que són els usuaris que han tingut un accident els que agafen por a la conducció, no al contràri. Està dient doncs que els promitjos no donen informació rellevant perquè els efectes es contraresten, però que mirant els quartils es pot notar la direcció de creixement o decreixement de les variables dependents en funció del grup. 
...
nonparam <- function(Y00,Y01,Y10,Y11,qq) {
  ...
  # Bootstrap
  boot = rep(0, length(est_con))
  Nboot = 100
  boot_est = matrix(0, Nboot, 1+length(qq))
  N00=length(Y00)
  N01=length(Y01)
  N10=length(Y10)
  N11=length(Y11)
  ...
  for(i in 1:Nboot) {
    ...
    # Generate random integers u
    u <- floor(runif(N00) * N00+ 1) 
    Y00b <- Y00[u]
    u <- floor(runif(N01) * N01+ 1)
    Y01b <- Y01[u]
    u <- floor(runif(N10) * N10+ 1)
    Y10b <- Y10[u]
    u <- floor(runif(N11) * N11+ 1) 
    Y11b <- Y11[u]
    
    YSb <- supp(c(Y00b, Y01b, Y10b, Y11b))
    YS01b <- supp(Y01b)
    
    # Calculate f00b, f01b, f10b, f11b
    f00b <- prob(Y00b, YSb)
    f01b <- prob(Y01b, YSb)
    f10b <- prob(Y10b, YSb)
    f11b <- prob(Y11b, YSb)
    ...
    # Calculate boot_est
    boot_est[i,1:(1+length(qq))] <- cic_con(f00b, f01b, f10b, f11b, qq, YSb, YS01b)
  }
  
  # eventually bootstrap standard errors are calculated using the percentile method
  low <- round(Nboot * 0.025)
  high <- round(Nboot * 0.975)
  boot_est_sort <- apply(boot_est, 2, sort)
  se_boot <- (boot_est_sort[high,] - boot_est_sort[low,]) / (2 * 1.96)

  # Inferència estadística amb bootstrap
  z_stat_boot <- est_con[1] / se_boot[1]
  # Two-tailed test
  p_value_boot <- 2 * (1 - pnorm(abs(z_stat_boot)))  
  z_critical_boot <- qnorm((1 + 0.95) / 2)
  CI_lower_boot <- est_con[1] - z_critical_boot * se_boot[1]
  CI_upper_boot <- est_con[1] + z_critical_boot * se_boot[1]
  # One-tailed test 
  z_critical_o_boot <- qnorm(1 - (1 - 0.95))
  if (direction == '>') {
    p_value_o_boot <- 1 - pnorm(z_stat_boot)  
    CI_lower_o_boot <- est_con[1] - z_critical_o_boot * se_boot[1]
    CI_upper_o_boot <- Inf # Positive infinity for greater than
  } else {
    p_value_o_boot <- pnorm(z_stat_boot) 
    CI_lower_o_boot <- -Inf  # Negative infinity for less than
    CI_upper_o_boot <- est_con[1] + z_critical_o_boot * se_boot[1]
  }
  
  # Inferència estadística dels quartils
  z_stat_boot_q <- est_con[2:length(est_con)]/ se_boot[2:length(est_con)]
  # Two-tailed test
  p_value_boot_q <- 2 * (1 - pnorm(abs(z_stat_boot_q)))  
  z_critical_boot_q <- qnorm((1 + 0.95) / 2)
  CI_lower_boot_q <- est_con[2:length(est_con)] - z_critical_boot_q * se_boot[2:length(est_con)]
  CI_upper_boot_q <- est_con[2:length(est_con)] + z_critical_boot_q * se_boot[2:length(est_con)]
  # One-tailed test 
  z_critical_o_boot_q <- qnorm(1 - (1 - 0.95))
  direction <- ifelse(est_con[2:length(est_con)] >0, ">", "<")
  direction <- direction[1]
  if (direction == '>') {
    p_value_o_boot_q <- 1 - pnorm(z_stat_boot_q)  
    CI_lower_o_boot_q <- est_con[2:length(est_con)]- z_critical_o_boot_q * se_boot[2:length(est_con)]
    CI_upper_o_boot_q <- Inf # Positive infinity for greater than
  } else {
    p_value_o_boot_q <- pnorm(z_stat_boot_q) 
    CI_lower_o_boot_q <- -Inf  # Negative infinity for less than
    CI_upper_o_boot_q <- est_con[2:length(est_con)] + z_critical_o_boot_q * se_boot[2:length(est_con)]
  }
  ...
  return(list(ATT_2 = est_con[1], sd_ATT = se_con, z_stat = z_stat,
              ...
              sd_ATT_boot = se_boot[1], z_stat_boot = z_stat_boot,
              p_value_boot = p_value_boot, CI_lower_boot = CI_lower_boot, CI_upper_boot = CI_upper_boot,
              p_value_o_boot = p_value_o_boot, CI_lower_o_boot = CI_lower_o_boot, CI_upper_o_boot = CI_upper_o_boot,
              quartils = est_con[2:length(est_con)],
              sd_ATT_boot_q = se_boot[2:length(est_con)], z_stat_boot_q = z_stat_boot_q,
              p_value_boot_q = p_value_boot_q, CI_lower_boot_q = CI_lower_boot_q, CI_upper_boot_q = CI_upper_boot_q,
              p_value_o_boot_q = p_value_o_boot_q, CI_lower_o_boot_q = CI_lower_o_boot_q, CI_upper_o_boot_q = CI_upper_o_boot_q))
}
```
...
```{r}
Y00 <- data1c[data1c$D == 0 & data1c$Time == 2,"Total"]
Y01 <- data1c[data1c$D == 0 & data1c$Time == 2,"Total"]
Y10 <- data1c[data1c$D == 1 & data1c$Time == 1,"Total"]
Y11 <- data1c[data1c$D == 1 & data1c$Time == 1,"Total"]
qq=c(0.25,0.5,0.75,0.9,0.95,0.99)

res_total <- nonparam(Y00, Y01, Y10, Y11, qq)

# Resultats: 
res_cic_tot<-cbind(ATT_2 = c(res_total$ATT_2, res_total$ATT_2, res_total$ATT_2, res_total$ATT_2), sd = c(res_total$sd_ATT, res_total$sd_ATT, res_total$sd_ATT_boot, res_total$sd_ATT_boot), z_stat = c(res_total$z_stat, res_total$z_stat, res_total$z_stat_boot, res_total$z_stat_boot), p_value = c(res_total$p_value, res_total$p_value_o, res_total$p_value_boot, res_total$p_value_o_boot), CI_lower = c(res_total$CI_lower, res_total$CI_lower_o,res_total$CI_lower_boot, res_total$CI_lower_o_boot), CI_upper = c(res_total$CI_upper, res_total$CI_upper_o,res_total$CI_upper_boot, res_total$CI_upper_o_boot))
rownames(res_cic_tot) <- c("2-tails", "1-tail", "2-tails bootstrap", "1-tail bootstrap")

knitr::kable(res_cic_tot,digits=4, caption="Changes-in-changes for Total variable")

res_quartils_tot<-cbind(ATT_2 = res_total$quartils, sd = res_total$sd_ATT_boot_q, z_stat = res_total$z_stat_boot_q, p_value =res_total$p_value_o_boot_q, CI_lower = res_total$CI_lower_o_boot_q, CI_upper = res_total$CI_upper_o_boot_q)
rownames(res_quartils_tot) <- qq
knitr::kable(res_quartils_tot,digits=4, caption="Changes-in-changes for quartiles - Total variable")
```

#### Variable Night

```{r}
Y00 <- data1c[data1c$D == 0 & data1c$Time == 2,"Night"]
Y01 <- data1c[data1c$D == 0 & data1c$Time == 2,"Night"]
Y10 <- data1c[data1c$D == 1 & data1c$Time == 1,"Night"]
Y11 <- data1c[data1c$D == 1 & data1c$Time == 1,"Night"]
qq=c(0.25,0.5,0.75,0.9,0.95,0.99)

res_total <- nonparam(Y00, Y01, Y10, Y11, qq)

# Resultats: 
res_cic_nig<-cbind(ATT_2 = c(res_total$ATT_2, res_total$ATT_2, res_total$ATT_2, res_total$ATT_2), sd = c(res_total$sd_ATT, res_total$sd_ATT, res_total$sd_ATT_boot, res_total$sd_ATT_boot), z_stat = c(res_total$z_stat, res_total$z_stat, res_total$z_stat_boot, res_total$z_stat_boot), p_value = c(res_total$p_value, res_total$p_value_o, res_total$p_value_boot, res_total$p_value_o_boot), CI_lower = c(res_total$CI_lower, res_total$CI_lower_o,res_total$CI_lower_boot, res_total$CI_lower_o_boot), CI_upper = c(res_total$CI_upper, res_total$CI_upper_o,res_total$CI_upper_boot, res_total$CI_upper_o_boot))
rownames(res_cic_nig) <- c("2-tails", "1-tail", "2-tails bootstrap", "1-tail bootstrap")

knitr::kable(res_cic_nig,digits=4, caption="Changes-in-changes for Night variable")

res_quartils_nig<-cbind(ATT_2 = res_total$quartils, sd = res_total$sd_ATT_boot_q, z_stat = res_total$z_stat_boot_q, p_value =res_total$p_value_o_boot_q, CI_lower = res_total$CI_lower_o_boot_q, CI_upper = res_total$CI_upper_o_boot_q)
rownames(res_quartils_nig) <- qq
knitr::kable(res_quartils_nig,digits=4, caption="Changes-in-changes for quartiles - Night variable")
```

#### Variable Speed

```{r}
Y00 <- data1c[data1c$D == 0 & data1c$Time == 2,"Speed"]
Y01 <- data1c[data1c$D == 0 & data1c$Time == 2,"Speed"]
Y10 <- data1c[data1c$D == 1 & data1c$Time == 1,"Speed"]
Y11 <- data1c[data1c$D == 1 & data1c$Time == 1,"Speed"]
qq=c(0.25,0.5,0.75,0.9,0.95,0.99)

res_total <- nonparam(Y00, Y01, Y10, Y11, qq)

# Resultats: 
res_cic_spe<-cbind(ATT_2 = c(res_total$ATT_2, res_total$ATT_2, res_total$ATT_2, res_total$ATT_2), sd = c(res_total$sd_ATT, res_total$sd_ATT, res_total$sd_ATT_boot, res_total$sd_ATT_boot), z_stat = c(res_total$z_stat, res_total$z_stat, res_total$z_stat_boot, res_total$z_stat_boot), p_value = c(res_total$p_value, res_total$p_value_o, res_total$p_value_boot, res_total$p_value_o_boot), CI_lower = c(res_total$CI_lower, res_total$CI_lower_o,res_total$CI_lower_boot, res_total$CI_lower_o_boot), CI_upper = c(res_total$CI_upper, res_total$CI_upper_o,res_total$CI_upper_boot, res_total$CI_upper_o_boot))
rownames(res_cic_spe) <- c("2-tails", "1-tail", "2-tails bootstrap", "1-tail bootstrap")

knitr::kable(res_cic_spe,digits=4, caption="Changes-in-changes for Speed variable")

res_quartils_spe<-cbind(ATT_2 = res_total$quartils, sd = res_total$sd_ATT_boot_q, z_stat = res_total$z_stat_boot_q, p_value =res_total$p_value_o_boot_q, CI_lower = res_total$CI_lower_o_boot_q, CI_upper = res_total$CI_upper_o_boot_q)
rownames(res_quartils_spe) <- qq
knitr::kable(res_quartils_spe,digits=4, caption="Changes-in-changes for quartiles - Speed variable")
```

#### Variable Urban

```{r}
Y00 <- data1c[data1c$D == 0 & data1c$Time == 2,"Urban"]
Y01 <- data1c[data1c$D == 0 & data1c$Time == 2,"Urban"]
Y10 <- data1c[data1c$D == 1 & data1c$Time == 1,"Urban"]
Y11 <- data1c[data1c$D == 1 & data1c$Time == 1,"Urban"]
qq=c(0.25,0.5,0.75,0.9,0.95,0.99)

res_total <- nonparam(Y00, Y01, Y10, Y11, qq)

# Resultats: 
res_cic_urb<-cbind(ATT_2 = c(res_total$ATT_2, res_total$ATT_2, res_total$ATT_2, res_total$ATT_2), sd = c(res_total$sd_ATT, res_total$sd_ATT, res_total$sd_ATT_boot, res_total$sd_ATT_boot), z_stat = c(res_total$z_stat, res_total$z_stat, res_total$z_stat_boot, res_total$z_stat_boot), p_value = c(res_total$p_value, res_total$p_value_o, res_total$p_value_boot, res_total$p_value_o_boot), CI_lower = c(res_total$CI_lower, res_total$CI_lower_o,res_total$CI_lower_boot, res_total$CI_lower_o_boot), CI_upper = c(res_total$CI_upper, res_total$CI_upper_o,res_total$CI_upper_boot, res_total$CI_upper_o_boot))
rownames(res_cic_urb) <- c("2-tails", "1-tail", "2-tails bootstrap", "1-tail bootstrap")
knitr::kable(res_cic_urb,digits=4, caption="Changes-in-changes for Urban variable")

res_quartils_urb<-cbind(ATT_2 = res_total$quartils, sd = res_total$sd_ATT_boot_q, z_stat = res_total$z_stat_boot_q, p_value =res_total$p_value_o_boot_q, CI_lower = res_total$CI_lower_o_boot_q, CI_upper = res_total$CI_upper_o_boot_q)
rownames(res_quartils_urb) <- qq
knitr::kable(res_quartils_urb,digits=4, caption="Changes-in-changes for quartiles - Urban variable")
```
